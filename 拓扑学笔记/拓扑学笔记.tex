...
\chapter{拓扑空间}
\section{拓扑空间，开集}
\begin{definition}[拓扑空间，开集]\label{c1-d1}\index{拓扑空间}\index{开集}
...
\begin{definition}[度量空间]\index{度量空间}
...
\begin{definition}[子空间拓扑]\index{子空间拓扑}
...
\end{proposition}
\subsection{闭集之刻画}
\begin{definition}[极限点]\index{极限点}
	设$X$是一个拓扑空间，$A\subset X$，$\forall p\in X$，若$\forall $包含$p$的开集$U$都有：
	\begin{equation*}
		(U\chadiao \dkh{p})\cap A \neq \emptyset
	\end{equation*}
	则称$p$为$A$的一个\gn{极限点}。而将集合$\overline{A} = A \cup \dkh{A\text{的极限点}}$称为$A$的\gn{闭包}。\index{闭包}
\end{definition}
...
\subsection{集合之解体}
为了更好说话，引入以下概念：
\begin{definition}[内点，外点，边界点]\index{内点}\index{外点}\index{边界点}
...
\end{equation*}
\end{note}
\begin{example}[一些显然的例子]\label{c1-l18}
	\begin{enumerate}
...
\end{definition}
\begin{example}[$\mathbb{R}^{1}$中的欧氏拓扑可以有不同的拓扑基]\label{c1-l19}
\label{c1-e19}
...
\end{proof}
\begin{example}[$\mathbb{R}^{2}$的另一拓扑基]\label{c1-l20}
...
\begin{definition}[连续映射]\index{连续性}
...
\begin{definition}[极限]\label{c2-d2}\index{极限}
...
\begin{definition}[Hausdorff空间]\index{Hausdorff空间}
...
\end{example}
\begin{example}[度量空间都是Hausdorff空间]\label{c2-l6}
...
\begin{example}[存在连续映射之逆映射不连续]\label{c2-l7}
设映射为：
\begin{equation*}
	\begin{aligned}
...
本节将给出一个在分析学、拓扑学中都很重要且著名的例子---Peano曲线。\index{Peano曲线}
...
\end{tikzpicture}
\caption{例\ref{c3-l1}中的$U_1$与$U_2$}
	\end{figure}
	
	即：$S^{2} = U_{1}\cup U_{2}$。
\end{example}

\begin{definition}[子覆盖]\index{子覆盖}
...
\begin{definition}[紧子集]\index{紧子集}
...
\begin{example}[$\mathbb{R}^{2}$中的闭矩形]
	比如集合$[a,b]\times [c,d] = \dkh{(x,y)|a\leq x\leq b,c\leq y\leq d}$，如图\ref{c3-t3}所示：\newpage
	\begin{figure}[h]
	\centering
...
\centering
\tikzset{every picture/.style={line width=0.75pt}}
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1,scale=0.88]
...
\end{tikzpicture}
	...
	在小圆中，建立点$(x,y,z)$与$z$轴之间的夹角为$\alpha$，这样我们可以看出映射$f$：
\begin{equation*}
	\begin{aligned}
...
\end{example}
下面我们给出乘积拓扑的定义：
\begin{definition}[乘积拓扑]\index{乘积拓扑}
...
\begin{definition}[范畴]\index{范畴}\index{态射}\index{对象}
...
\end{proof}
\section{更多的紧性}\subsection{极限点紧性}
\begin{definition}[极限点紧性]\index{极限点紧性}
...
%		但是对于$\mathbb{Z}_{+}\times Y$的一组拓扑基为$\dkh{n}\times Y,n=1,2,\cdots$，则$\forall U\subset_{open}\mathbb{Z}_{+}\times Y,(n,0)\in U$
	\end{proof}\end{example}\subsection{列紧性}
	\begin{definition}[列紧性]\index{列紧性}
...
\end{example}
\begin{example}[圆扣掉两个点就不连通了]\label{c4-l3}
...
\end{tikzpicture}\caption{例\ref{c4-l3}中的$X = S^{1}\chadiao\dkh{P,Q}$}
	\end{figure}
\begin{definition}[连通性]\index{连通性}
...
\begin{example}[一维单位球面与$\mathbb{R}$不是同胚的]\label{c4-l4}
	$S^{1}\ncong \mathbb{R}$
\end{example}
...
\begin{example}[$\mathbb{R}$与其上开区间是同胚的]\label{c4-l5}
	$(0,1)\cong \mathbb{R}$
\end{example}
\begin{proof}
	直接取连续映射$f$如下：
...
\begin{definition}[连通分支]\index{连通分支}
...
\end{proof}	
\end{note}
\begin{example}[不连通集$\mathbb{Q}$的连通分支]\label{c4-l11}
...
\end{definition}
\subsection{道路连通性与例}
\begin{definition}[道路连通]\index{道路连通性}
...
\draw [fill={rgb, 255:red, 252; green, 211; blue, 55 }  ,fill opacity=1 ]   (165,66) .. controls (205,36) and (233,55.5) .. (245,59.5) .. controls (257,63.5) and (323,101.5) .. (262,125.5) .. controls (201,149.5) and (140,91) .. (165,66) -- cycle ;
%Curve Lines [id:da5888643693255495] 
\draw [fill={rgb, 252:red, 243; green, 211; blue, 55 }  ,fill opacity=1 ]   (336,159.5) .. controls (352,126.5) and (402,117.5) .. (420,120.5) .. controls (438,123.5) and (482,165.5) .. (421,189.5) .. controls (360,213.5) and (311,184.5) .. (336,159.5) -- cycle ;
...
\end{proof}
\begin{example}[连通但不道路连通的例子---拓扑学家的正弦曲线]\label{c4-l13}\index{拓扑学家的正弦曲线}
	设拓扑空间$X \subset \R^{2} $赋予子空间拓扑，且$X :=Y\sqcup Z$，其中：
	\begin{figure}[h]
		\centering
		\includegraphics[scale=0.6]{图片/sin_1.pdf}
		\caption{例\ref{c4-l13}之函数图像：\gn{拓扑学家的正弦曲线}}
	\end{figure}
	\begin{equation*}
...
\begin{proof}
	...
	\begin{figure}[h]
		...
		\caption{例\ref{c4-l13}之$U\cap X$（放大部分）}
	\end{figure}
...
道路连通的连通分支的闭包不必为闭集。	
\end{note}
\begin{definition}[局部道路连通]\index{局部道路连通}
...
\begin{lemma}[Glueing引理]\label{c4-y1}\index{Glueing引理}
